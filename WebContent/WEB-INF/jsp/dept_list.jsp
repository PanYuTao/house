	...
	<div class="rightinfo">
		...
		<thead>
			<tr align='center'>
				...
				<th width='25%' align='center'>部门备注</th>
				<th width='35%' align='center'>部门名称</th>
				<th width='15%'>状态</th>
				<th width='15%'>操作</th>
			</tr>
		</thead>
			<c:forEach items="${deptlList }" var="dept">
				<tr>
					<td>${dept.pid }</td>
					<td>${dept.pname }</td>
					<td>${dept.pflag }</td>
					<td>${dept.premark }</td>
					<td>
						<a href="Demo?type=deptSelectByid&pid=${dept.pid }">修改</a>
						<a href="javaScript:deptDel('Demo?type=deptDel&pid=${dept.pid }')">删除</a>
					</td>
				</tr>
			</c:forEach>
		</table>
