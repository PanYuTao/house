...
function getAll(current) {
	...
	$(".tablelist").empty();
	$(".tablelist").append("<thead><tr align='center'><th width='10%' align='center'>编号</th><th width='25%' align='center'>合同名称</th><th width='15%' align='center'>操作</th></tr></thead>");
	
    $.ajax({
    	url:'http://localhost:8080/house2023/HtController?type=list',//请求地址
    	dataType:'json',//指定服务器响应数据类型,将字符串转成json对象
    	data:{current:current},//携带参数
    	type:'post',//提交方式
    	success:function(mydata)//mydata封装服务响应数据
    	{
    		...
    		if(mydata=="9999")
	    		...
	    		$.each(mydata.list,function(index,xx){
					    ...
					    srt+='<td><a href="#" class="myupp">修改合同</a>&nbsp;';
					    srt+='<a href="#" class="mydel">删除合同</a></td></tr>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			sort : [ '300px', '100px' ],
			content : 'http://localhost:8080/house2023/ht/update.html?htid='+htid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post($webName+'/HtController?type=delete&htid='+htid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
