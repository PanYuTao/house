...
function getAll(current,flag) {
	...
	$.ajax({
    	url:'http://localhost:8080/house2023/SfController?type=list',
    	dataType:'json',
...
function outupdate()
{
	$(".myupp").live("click",function(){
		var yid = $(this).parents("tr").children("td").eq(0).text();
		layer.open({
			type:2,
			title:'修改出租信息',
			fix:false,
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : $webName+'/djsf/update.html?yid='+yid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post($webName+'/SfController?type=delete&yid='+yid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
