...
function getAll(current) {
    ...
    $.ajax({
    	url:'http://localhost:8080/house2023/ZcController?type=list',
    	dataType:'json',
    	...
    	success:function(mydata)
    	{
    		if(mydata=="9999")
        		...
        		$.each(mydata.list,function(index,xx){
				    ...
				    srt+='<a href="#" class="mydel">删除员工</a></td></tr>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		var cid = $(this).parents("tr").children("td").eq(0).text();
		window.location.href=$webName+"/zc/update.html?cid="+cid;
	/*	layer.open({
			type:2,
			title:'修改政策',
			fix:false,
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post($webName+'/ZcController?type=delete&cid='+cid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
