...
function getAll(current,flag) {
    ...
    $.ajax({
    	url:'http://localhost:8080/house2023/DeptController?type=list',
    	dataType:'json',
    	data:{current:current,pflag:flag},
    	type:'post',
    	success:function(mydata)//mydata是服务器返回的pageInfo对象
    	{
    		...
    		if(mydata=="9999")
    			...
    			$.each(mydata.list,function(index,xx){
			    	    ...
			    	    srt+='<a href="javascript:void(0)" class="mydel">删除</a></td></tr>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : 'http://localhost:8080/house2023/part/update.html?pid='+pid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/house2023/DeptController?type=delete&pid='+pid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
