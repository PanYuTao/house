...
function getAll(current,flag) {
    ...
    $.ajax({
    	url:'http://localhost:8080/house2023/EmpController?type=list',
    	dataType:'json',
    	...
    	success:function(mydata)
    	{
    		...
    		$.each(mydata.list,function(index,xx){
				    ...
				    if(flag==0)
				    	{
				    	  srt+='<td><a href="javascript:void(0)" class="myupp">修改</a></td>';
				    	  srt+='<td><a href="javascript:void(0)" class="mydel">删除员工</a></td></tr>';
				    	}
				    else
				    	{
				    	    srt+='<td><a href="javascript:void(0)" class="backdel">恢复员工</a></td></tr>';
				    	}
...
function addEmp(){
	...
	$(".addPart").bind("click",function() {
		layer.open( {
			...
			shadeClose : true,
			area : [ '300px', '300px' ],
			content : 'http://localhost:8080/house2023/emp/add.html',
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : 'http://localhost:8080/house2023/emp/update.html?eid='+eid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/house2023/EmpController?type=delete&eid='+eid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
