...
function initJs() {
	...
	//得到下拉框的内容
	$.ajax({
		url:'http://localhost:8080/house2023/SortController?type=listAll',
		dataType:'json',
		type:'post',
		data:'',
		async : true,
...
function myaddSe()
{
	$("#aid").bind("change",function(){
		...
		if(aid!=0)
			{
					$.ajax({
					url:'http://localhost:8080/house2023/HouseController?type=listAll',
					dataType:'json',
					type:'post',
...
function getAll(current,aid,sid,hid) {
    ...
    $.ajax({
    	url:'http://localhost:8080/house2023/HouseController?type=list2',
    	dataType:'json',
...
function getAll1(current,mid) {
	...
	$.ajax({
    	url:'http://localhost:8080/house2023/Zsrall2Controller?type=list2',
    	dataType:'json',
...
function getBack()
{
	$("#x5").click(function(){
		history.back();
		   window.location.href="http://localhost:8080/house2023/zsr/all2.html";
