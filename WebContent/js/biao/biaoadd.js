...
function init() {
	...
	$.ajax({
		...
		{
		   $.each(mydata,function(index,xx){
			   $("#eid").append("<option value="+xx.eid+">"+xx.ename+"</option>");
		   });
		}
	});
};
...
function commitItem()
{
	...
	else if(eid==0)
	{
	   ...
	   return false;
		...
		{
		   var mypart = "biao.skd="+skd+"&biao.dkd="+dkd+"&biao.mkd="+mkd+"&biao.hid="+hid+"&biao.eid="+eid;
		   var i = layer.load(0);
		   $.post($webName+'/BiaoController?type=add',mypart,function(mydata){
			 layer.close(i);
			 if(mydata==1)
				 {
