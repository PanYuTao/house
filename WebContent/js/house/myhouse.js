...
function getAll(current,id) {
	...
	$(".tablelist").append("<thead><tr align='center'><th align='center'>编号</th><th align='center'>房屋类型</th><th align='center'>片区</th><th align='center'>房子地址</th><th align='center'>房号</th><th align='center'>房子户型</th><th align='center'>面积</th><th align='center'>朝向</th><th align='center'>原定最低租金</th><th align='center'>出租状态</th><th  align='center'>操作</th></tr></thead>");
	
    $.ajax({
    	url:$webName+'/HouseController?type=list',
    	dataType:'json',
    		...
    		else
    			{
	    			...
	    			$.each(mydata.list,function(index,xx){
		    				   ...
		    				   // srt+='<td>'+xx.mkd+'</td>';
		    				    ...
		    				    srt+='<a href="#" class="mydel">删除</a></td></tr>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			sort : [ '300px', '100px' ],
			content : '../house/update.html?hid='+hid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post($webName+'/HouseController?type=delete&hid='+hid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
...
function getHuiFu()
{
	$(".backdel").live("click",function(){
		...
		layer.confirm('你确定要恢复此部门吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('${pageContext.request.contextPath}/part_backdel.action?hid='+hid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
