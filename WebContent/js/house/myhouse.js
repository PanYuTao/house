...
function getAll(current,id) {
    ...
    $.ajax({
    	url:'http://localhost:8080/house2023/HouseController?type=list',
    	dataType:'json',
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/house2023/HouseController?type=del&hid='+hid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
