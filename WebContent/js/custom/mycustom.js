...
function getAll(current) {
	...
	$(".tablelist").empty();
	$(".tablelist").append("<thead><tr style='text-align: center;'><th  align='center'>编号</th><th  align='center'>客户姓名</th><th  align='center'>性别</th><th align='center'>电话</th><th  align='center'>备用电话</th><th  align='center'>身份证号</th><th  align='center'>操作</th></tr></thead>");
	
    $.ajax({
    	url:'http://localhost:8080/house2023/CusController?type=list',
    	dataType:'json',
    	...
    	if(mydata=="9999")
    		...
    		$.each(mydata.list,function(index,xx){
			    ...
			    srt+='<a href="javascript:void(0)" class="mydel">删除员工</a></td></tr>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '100px' ],
			content : 'http://localhost:8080/house2023/custom/update.html?cid='+cid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/house2023/CusController?type=delete&cid='+cid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
