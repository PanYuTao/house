...
function init() {
	...
	$.ajax({
		url:'http://localhost:8080/house2023/CustomController?type=toUpdate',
		data:{cid:cid},
		dataType:'json',
		type:'post',
...
function commitItem()
{
	...
	if(cname.length==0)
	...
	else
		{
		    ...
		    var i = layer.load(0);
		   $.post('http://localhost:8080/house2023/CustomController?type=update',mypart,function(mydata){
			 layer.close(i);
			 if(mydata==1)
				 {
