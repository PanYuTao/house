...
function getAll(current) {
    ...
    $.ajax({
    	url:'http://localhost:8080/house2023/ZhichuController?type=list',
    	dataType:'json',
    	...
    	success:function(mydata)
    	{
    		if(mydata=="9999")
        		...
        		$.each(mydata.list,function(index,xx){
				    ...
				    srt+='<a href="#" class="mydel">删除</a></td></td>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			sr : [ '300px', '100px' ],
			content : '../zhichu/update.html?zid='+zid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post($webName+'/ZhichuController?type=delete&zid='+zid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
