...
function init() {
	...
	//查询基本信息
	$.ajax( {
		url : $webName+'/BsController?type=toUpdate',
		data : {
			bid : bid
		},
		dataType : 'json',
...
function commitItem() {
   $(".btn").bind("click",function(){
	...
	if(hid.length==0)
		...
		{
		   var mypart = "bs.hid="+hid+"&bs.bremark="+bremark+"&bs.bid="+bid+"&bs.eid="+eid;
		   var i = layer.load(0);
		   $.post($webName+'/BsController?type=update',mypart,function(mydata){
			 layer.close(i);
			 if(mydata==1)
				 {
