	...
	if(flag==0)
		{
		   ...
		   $(".tablelist").append("<thead><tr align='center'><th  align='center'>编号</th><th align='center'>房屋地址</th><th  align='center'>房号</th><th  align='center'>客户姓名</th><th  align='center'>客户电话</th><th  align='center'>经办人</th><th  align='center'>登记时间</th><th>退租截止日期</th><th>续费过程明细</th></tr></thead>");
		       /****************开始****************/
		       	...
		       	$.ajax({
				    	url:'http://localhost:8080/house2023/DjController?type=list',
				    	dataType:'json',
		...
		{
		   //正在出租
		   $(".tablelist").append("<thead><tr align='center'><th  align='center'>编号</th><th align='center'>房屋地址</th><th  align='center'>房号</th><th  align='center'>客户姓名</th><th  align='center'>客户电话</th><th  align='center'>经办人</th><th  align='center'>登记时间</th><th  align='center'>押金</th><th >预收租金</th><th>房屋状态</th><th>下次收租日期</th><th>操作</th></tr></thead>");
		   $.ajax({
    	   url:'http://localhost:8080/house2023/DjController?type=list',
    	   dataType:'json',
    	   data:{current:current,hflag:flag},
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : 'http://localhost:8080/house2023/djrz/update.html?mid='+mid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post($webName+'/DjController?type=delete&mid='+mid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
